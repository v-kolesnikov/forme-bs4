...
describe "Forme Bootstrap4 (BS4) forms" do
  ...
  it "should create a simple input tags" do
    ...
    @f.input(:submit).to_s.must_equal '<input class="btn btn-primary" type="submit"/>'
  end
  ...
  it "should respect the :error option for checkbox sets" do
    @f.input(:checkboxset, :options=>[1, 2, 3], :error=>'foo-checkboxset', :value=>2).to_s.must_equal '<div class="checkboxset has-error"><div class="checkbox"><label class="option"><input type="checkbox" value="1"/> 1</label></div><div class="checkbox"><label class="option"><input checked="checked" type="checkbox" value="2"/> 2</label></div><div class="checkbox"><label class="option"><input type="checkbox" value="3"/> 3</label></div><span class="form-text with-errors">foo-checkboxset</span></div>'
  end
  ...
  it "should automatically note the input has errors if :error option is used" do
    @f.input(:text, :error=>'Bad Stuff!', :value=>'foo').to_s.must_equal '<div class="form-group has-error"><input class="form-control" type="text" value="foo"/><span class="form-text with-errors">Bad Stuff!</span></div>'
  end

  it "should add an error message after the label" do
    @f.input(:text, :error=>'Bad Stuff!', :value=>'foo', :label=>"Foo").to_s.must_equal '<div class="form-group has-error"><label>Foo</label> <input class="form-control" type="text" value="foo"/><span class="form-text with-errors">Bad Stuff!</span></div>'
  end

  it "should add to existing :class option if :error option is used" do
    @f.input(:text, :error=>'Bad Stuff!', :class=>'bar', :value=>'foo').to_s.must_equal '<div class="form-group has-error"><input class="form-control bar" type="text" value="foo"/><span class="form-text with-errors">Bad Stuff!</span></div>'
  end

  it "should respect :error_attr option for setting the attributes for the error message span" do
    @f.input(:text, :error=>'Bad Stuff!', :value=>'foo', :error_attr=>{:class=>'foo'}).to_s.must_equal '<div class="form-group has-error"><input class="form-control" type="text" value="foo"/><span class="foo form-text with-errors">Bad Stuff!</span></div>'
  end
  ...
  it "#button should return a submit tag" do
    @f.button.to_s.must_equal '<input class="btn btn-primary" type="submit"/>'
  end

  it "#button should return a submit tag without label" do
    @f.button(:label=>'foo').to_s.must_equal '<input class="btn btn-primary" type="submit"/>'
  end

  it "#button should accept an options hash" do
    @f.button(:name=>'foo', :value=>'bar').to_s.must_equal '<input class="btn btn-primary" name="foo" type="submit" value="bar"/>'
  end
  ...
  it "#button should accept a string to use as a value" do
    @f.button('foo').to_s.must_equal '<input class="btn btn-primary" type="submit" value="foo"/>'
  end
  ...
  it "inputs should have helper displayed inside wrapper, after error" do
    @f.input(:text, :help=>"List type of foo", :error=>'bad', :wrapper=>:li).to_s.must_equal '<li class="has-error"><input class="form-control" type="text"/><span class="form-text with-errors">bad</span><span class="helper">List type of foo</span></li>'
  end
  ...
  it "inputs should accept a :formatter option to use a custom formatter" do
    ...
    @f.input(:text, :formatter=>:bs4_readonly, :value=>'1', :label=>'Foo').to_s.must_equal '<div class="form-group"><label>Foo</label> <input class="form-control" readonly="readonly" type="text" value="1"/></div>'
  end

  it "bs4_readonly formatter should disable checkbox, radio, select, and textarea inputs" do
    @f.input(:checkbox, :formatter=>:bs4_readonly).to_s.must_equal '<div class="checkbox"><input disabled="disabled" type="checkbox"/></div>'
    @f.input(:radio, :formatter=>:bs4_readonly).to_s.must_equal '<div class="radio"><input disabled="disabled" type="radio"/></div>'
    @f.input(:select, :formatter=>:bs4_readonly).to_s.must_equal '<div class="form-group"><select class="form-control" disabled="disabled"></select></div>'
    @f.input(:textarea, :formatter=>:bs4_readonly).to_s.must_equal '<div class="form-group"><textarea class="form-control" readonly="readonly"></textarea></div>'
  end
  ...
  it "#inputs should accept a :inputs_wrapper option to use a custom inputs_wrapper" do
    ...
    @f.inputs([:textarea], :inputs_wrapper=>:bs4_table, :wrapper=>:trtd).to_s.must_equal '<table class="table table-bordered"><tr><td><textarea class="form-control"></textarea></td><td></td></tr></table>'
    @f.inputs([:textarea], :inputs_wrapper=>:bs4_table, :wrapper=>:trtd, :legend=>'Foo', :labels=>['bar']).to_s.must_equal '<table class="table table-bordered"><caption>Foo</caption><tr><th>bar</th></tr><tr><td><textarea class="form-control"></textarea></td><td></td></tr></table>'
  end

  it "inputs should accept a :wrapper=>nil option to not use a wrapper" do
    Forme::Form.new(:config=>:bs4,:wrapper=>:li).input(:text, :wrapper=>nil).to_s.must_equal '<input class="form-control" type="text"/>'
  end
  ...
  it "should support setting defaults for inputs at the form level" do
    f = Forme::Form.new(:config=>:bs4, :input_defaults=>{'text'=>{:size=>20}, 'textarea'=>{:cols=>80, :rows=>6}})
  ...
  it "should work with input_defaults with symbol keys using using inputs with symbol keys" do
    f = Forme::Form.new(:config=>:bs4, :input_defaults=>{:text=>{:size=>20}, 'text'=>{:size=>30}})
    f.input(:text, :name=>"foo").to_s.must_equal '<div class="form-group"><input class="form-control" name="foo" size="20" type="text"/></div>'
  ...
  it "invalid custom transformers should raise an Error" do
    proc{Forme::Form.new(:config=>:bs4, :wrapper=>Object.new).input(:text).to_s}.must_raise(Forme::Error)
